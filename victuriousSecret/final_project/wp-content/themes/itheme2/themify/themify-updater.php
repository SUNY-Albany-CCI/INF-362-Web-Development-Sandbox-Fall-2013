...
function themify_check_version(){
	...
	if ( isset( $_GET['success'] ) && 'true' == $_GET['success'] && 'theme' == $_GET['upgrade_type'] ) {
		$notifications .= '<p class="success">' . sprintf( __('Successfully updated %s to version %s.', 'themify'), $theme_name, $theme_version ) . '</p>';	
	}
	if ( isset( $_GET['success'] ) && 'true' == $_GET['success'] && 'framework' == $_GET['upgrade_type'] ) {
	...
	if( is_object($newF) || is_object($newT) ){
		...
		if( is_object($newT) && ($theme_version < $newT->version) ){
			...
			if($_GET['page'] == 'themify')
				...
				$theme_notifications = '<p class="update">' . sprintf(__('%s version %s is now available. Visit the <a href="admin.php?page=themify">%s page</a> to update.', 'themify'), ucwords($theme_name), $newT->version, ucwords($theme_name)) . '</p>';
		}
		...
		if( is_object($newF) && ($version < $newF->version) ){
			...
			if($_GET['page'] == 'themify')
				...
				$fw_notifications = '<p class="update">' . sprintf( __('Framework version %s is now available. Visit the <a href="admin.php?page=themify">%s page</a> to update.', 'themify'), $newF->version, ucwords($theme_name)) . '</p>';
		}
		...
		foreach($versions['versions']['_c']['version'] as $update){
			...
			if($update['_a']['name'] == 'themify' && !is_object($newF)){
				...
				if(str_replace(".","",$theme_version) < $latest){
					...
					$theme_notifications = sprintf( __("<p class='update %s'>%s version %s is now available. <a href='%s' title='' class='%s' target='%s'>Update now</a> or view the <a href='http://themify.me/logs/%s-changelogs' title='' class='' target='_blank'>change log</a> for details.</p>", 'themify'), $login, ucwords($theme_name), $update['_v'], $url, $class, $target, $theme_name);
...
function themify_updater(){
	global $theme;
	$themeName = '' != $theme->display('Template')? $theme->display('Template') : strtolower($theme->display('Name'));
	$type = $_GET['type'];
