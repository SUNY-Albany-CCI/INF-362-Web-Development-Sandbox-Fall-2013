...
//add_shortcode('instagram', 'themify_shortcode_instagram');
//add_shortcode('themify-instagram', 'themify_shortcode_instagram');
...
function themify_shortcodes_js_css(){
	global $version, $themify_twitter_instance;
	...
	wp_register_script('themify-map-script', themify_https_esc('http://maps.google.com/maps/api/js').'?sensor=false', array(), $version, true);
...
function themify_shortcode_list_posts($atts, $content = null) {
	global $wpdb, $post, $table_prefix, $themify;
	wp_enqueue_script('themify-shortcodes-js');
	extract(shortcode_atts(array(
		'title' => 'yes',
		'category' => '0',
...
/**
 * Insert Instagram Gallery by user
 * @param Object $atts
 * @param String $content
 * @return String
 */	
function themify_shortcode_instagram($atts, $content = null) {
	extract(shortcode_atts(array(
		'user' => '',
		'limit' => '8',
		'is_widget' => 'false',
	...
	$userid = '';
	$is_shortcode = '';
	if('false' == $is_widget) $is_shortcode = 'shortcode';
	
	if('true' == $nocache) {
		delete_transient( 'themify_instagram_'.$user );
		delete_transient( 'themify_instagram_user_'.$user );
	}
	
	// Find out user id ///////////////////////////////////////////////////////////
	$userid = get_transient( 'themify_instagram_user_'.$user ); if ( empty( $userid ) ) {
		$response = wp_remote_get('https://api.instagram.com/v1/users/search?q='.$user.'&access_token=245329700.f59def8.b8f2618321284b7298074aeff398289a', array( 'sslverify' => false ));
		if( is_wp_error( $response ) ) {
			$error_message = $response->get_error_message();
			printf(__('Error returning Instagram user ID: %s', 'themify'), $error_message);
		} else {
			$json = json_decode($response['body']);
			$userid = $json->data[0]->id;
		}
	set_transient('themify_instagram_user_'.$user, $userid, HOUR_IN_SECONDS ); }
	
	// Get photos ///////////////////////////////////////////////////////////
	$out = get_transient( 'themify_instagram_'.$user ); if ( empty( $out ) ) {
		$response = wp_remote_get('https://api.instagram.com/v1/users/'.$userid.'/media/recent/?access_token=245329700.f59def8.b8f2618321284b7298074aeff398289a&count='.$limit, array( 'sslverify' => false ));
		
		if( is_wp_error( $response ) ) {
			$error_message = $response->get_error_message();
			printf(__('Error returning Instagram feed: %s', 'themify'), $error_message);
		} else {
			$json = json_decode($response['body']);
			for ($i = $limit-1; $i >= 0 ; $i--) {
				if('image' != $json->data[$i]->type) continue;
				$out .= '<li><a href="'.$json->data[$i]->link.'" title="'.esc_attr($json->data[$i]->caption->text).'"><img src="'.$json->data[$i]->images->thumbnail->url.'" width="'.$json->data[$i]->images->thumbnail->width.'" height="'.$json->data[$i]->images->thumbnail->height.'" /></a></li>';
			}
			if('' != $out) {
				$out = sprintf('
						<!-- Instagram -->
						<div class="%s clearfix themify-instagram">
							<ul>%s</ul>
						</div>',
						$is_shortcode,
						$out);
	...
	set_transient('themify_instagram_'.$user, $out, HOUR_IN_SECONDS ); }
...
function themify_request_tweets($args) {
	...
	$consumer_key = $data[$prefix.'consumer_key'];
	$consumer_secret = $data[$prefix.'consumer_secret'];
...
function themify_twitter_flush_transient( $post_id ) {
	...
	if ( !wp_is_post_revision( $post_id ) ) {
		...
		$unprefixed_shortcode = substr_count($_POST['content'], '[twitter');
		$prefixed_shortcode = substr_count($_POST['content'], '[themify-twitter');
